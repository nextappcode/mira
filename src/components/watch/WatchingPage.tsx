...
export const WatchingPage: React.FC<WatchingPageProps> = ({
  ...
  // Video + overlays + HUD — reused in both normal and fullscreen views
  const VideoContent = (
    <>
      <VideoView
        stream={remoteStream}
        ...
        {status.includes("Conectado") && !hasValidFrames && !isPaused && (
          ...
          >
            <button
              ...
              className="w-24 h-24 bg-[var(--p-500)] rounded-full flex items-center justify-center shadow-[var(--shadow-glow-p)] hover:scale-110 transition-transform animate-pulse"
            >
  ...
  return (
    <>
      ...
      */}
      ...
      >
        {/* Left column — video box (content hidden when fullscreen overlay is active) */}
        <div className="lg:col-span-9 flex flex-col shadow-2xl rounded-[var(--radius-xl)] overflow-hidden bg-black border border-[var(--border-subtle)] relative group">
          <div
            ref={isFullscreen ? undefined : videoContainerRef}
            className="w-full h-full relative"
          >
            {!isFullscreen && VideoContent}
          </div>
        </div>
